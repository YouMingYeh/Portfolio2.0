...
const MessengerAnimation = ({ messages, handleClick }) => {
    ...
    async function add() {
      document.querySelectorAll(".message").forEach((message, i) => {
        if (index > i)
          gsap.to(message, {
            duration: 1,
            transform: `translateY(-${3.6 * (index - i)}rem)`,
          });
      });
  ...
  return (
    <div
      className="p-4 h-full relative rounded-lg w-screen justify-end flex-col bottom-0 absolute transition-all"
      ref={containerRef}
    >
      {messages.map((m, i) => {
        if (i == messages.length - 1) {
          return (
            <div
              key={`${m}-${i}`}
              className="chat chat-start absolute self-start px-6 py-2 m-1"
            >
              <div
                id={`${m}-${i}`}
                className="chat-bubble message whitespace-nowrap hidden rounded-full opacity-0  text-2xl min-w-full font-bold bg-base-200 text-neutral"
                key={i}
              ></div>
            </div>
          );
        ...
        return (
          ...
          >
            <div
              id={`${m}-${i}`}
              className="chat-bubble message whitespace-nowrap hidden rounded-full opacity-0  text-2xl min-w-full bg-base-200 text-neutral"
              key={i}
            ></div>
          </div>
        );
      ...
      {pointing && (
        <div className="arrow opacity-70 absolute bottom-3 right-1/2  mb-8 hover:cursor-pointer">
          <a className="text-8xl" onClick={handleClick}>
            👇
          </a>
        </div>
      )}
